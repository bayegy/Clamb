...
class Weipu(object):
    ...
    def requests(self,*args,method="post",timeout=20,proxies=False,**kwargs) -> html.HtmlElement:
        ...
        while True:
            try:
                response=requests.post(*args,**kwargs) if method=="post" else requests.get(*args,**kwargs,timeout=timeout,proxies=proxies)
    ...
    def search_id(self,author=None,organ=None,title=None,only_first_author=False,page="1") -> []:
        ...
        # print(req_data)
        idset = self.requests(self.__url,req_data,headers=self.__headers,timeout=20).xpath("//@articleid")
    ...
    def find_author_email_in_article(self,author,article_id) -> []:
        tree=self.requests(method="get",timeout=20,headers=self.__get_headers,url="http://qikan.cqvip.com/Qikan/Article/Detail?id={}".format(article_id))
        # authors = tree.xpath('//div[@class="author"]/span/a/span/text()')
        author_info= ''.join(tree.xpath('//div[@class="others"]/text()')).strip().replace('．','.').replace('\n','').replace('\r','')
    ...
    def get_column(self,path,number=2,sep="\t"):
        return pd.read_csv(path,sep=sep).values if number==-1 else pd.read_csv(path,sep=sep).ix[:,0:number].values 
